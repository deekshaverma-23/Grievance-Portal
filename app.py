...
def user_page():
    ...
    st.markdown("Submit your complaint below and get an AI-generated resolution plan.")

    with st.form(key="grievance_form"):
        complaint_text = st.text_area(
            "Enter your complaint:",
            height=200,
            help="e.g., The streetlights in my locality are not working."
        )
    ...
    if submit and complaint_text:
        with st.spinner("Processing..."):
            try:
                app.invoke({"complaint_text": complaint_text})
                st.success("Your complaint has been submitted!")
                st.info("It has been assigned with priority and resolution for officials.")
...
def compute_urgency(row):
    ...
    return round(0.45*s + 0.30*p + 0.15*se + 0.10*c, 2)
...
def render_category_page(category):
    ...
    complaints = get_all_complaints()
    if not complaints:
        st.info("No complaints exist yet.")
        return
    ...
    if df.empty:
        st.info(f"No complaints under {category}.")
        return
